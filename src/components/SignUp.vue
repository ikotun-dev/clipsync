  ...
  <div class="h-screen bg-gray-950 flex flex-col items-center justify-center ">
    ...
    <div class=" mb-10 flex flex-col items-center justify-cente bg-gray-950">
      ...
      <div v-show="signupSucess === true">
        <h4 class="text-blue-400 font-montserrat font-extrabold text-sm lg:text-md mb-1 mt-1 "> Yayy, Spot reserved 🎉
        </h4>
      </div>
      ...
      <h3 class="text-gray-500 font-montserrat text-xs hidden lg:text-xs font-bold my-2 mb-3">Reserve your username and
        get notified when its your turn &#128521; </h3>
      <form>
        ...
        <h4 v-show="short_code == true" class="text-red-600 font-montserrat mt-2 font-extrabold text-xs">Password is less
          than 8 characters
          characters </h4>
        <h4 v-show="notAlpha == true" class="text-red-600 font-montserrat mt-2 font-bold text-xs">Code is not a valid
          character
          ! </h4>
        <h4 v-show="no_value == true" class="text-red-600 font-montserrat mt-2 font-extrabold text-xs">Enter a valid
          password </h4>

        <button v-if="processing == false" type="submit"
          ...
          @click="ValidateCredentials()">Sign up &#x1F680;</button>
  ...
  methods: {
    ...
    async signUp() {
      ...
      try {
        ...
        res = await axios.post('https://dropit-express.fly.dev/user/signup', userData, {
          headers: {
